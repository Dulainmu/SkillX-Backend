...
exports.startCareer = async (req, res) => {
  try {
    let { careerRoleId } = req.body;
    if (!careerRoleId) {
      return res.status(400).json({ message: 'careerRoleId is required.' });
    }
    
    // If not a valid ObjectId, treat as slug
    if (!mongoose.Types.ObjectId.isValid(careerRoleId)) {
      const career = await CareerRole.findOne({ slug: careerRoleId });
      if (!career) {
        return res.status(404).json({ message: 'Career role not found.' });
      }
      careerRoleId = career._id.toString();
    }
    ...
    // Initialize steps based on detailed roadmap if available, otherwise use basic roadmap
    let steps = [];
    if (career.detailedRoadmap && career.detailedRoadmap.length > 0) {
      steps = career.detailedRoadmap.map((_, idx) => ({ stepIndex: idx, completed: false }));
    } else if (career.roadmap && career.roadmap.length > 0) {
      steps = career.roadmap.map((_, idx) => ({ stepIndex: idx, completed: false }));
    } else {
      // Fallback: create at least one step
      steps = [{ stepIndex: 0, completed: false }];
    }
    ...
    console.error('Error in startCareer:', err);
...
exports.getProgress = async (req, res) => {
  try {
    let { careerRoleId } = req.params;
    
    // If not a valid ObjectId, treat as slug
    if (!mongoose.Types.ObjectId.isValid(careerRoleId)) {
      const career = await CareerRole.findOne({ slug: careerRoleId });
      if (!career) {
        return res.status(404).json({ message: 'No career found for this slug.' });
      }
      careerRoleId = career._id.toString();
    }
    ...
    if (!progress) {
      return res.status(404).json({ message: 'No progress found for this career.' });
    }
...
exports.updateStep = async (req, res) => {
  try {
    let { careerRoleId, stepIndex } = req.params;
