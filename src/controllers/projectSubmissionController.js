...
exports.submitProject = async (req, res) => {
  try {
    ...
    if (!careerRoleId || stepIndex === undefined || !projectId || !title) {
      console.log('Missing required fields:', { careerRoleId, stepIndex, projectId, title });
    ...
    // Find step and project details from career roadmap
    let stepTitle = `Step ${stepIndex}`;
    let projectTitle = title;
    
    if (career && Array.isArray(career.detailedRoadmap)) {
      const step = career.detailedRoadmap.find(s => s.id === `step-${stepIndex}` || s.stepIndex === stepIndex);
      if (step) {
        stepTitle = step.title || stepTitle;
        const project = step.projects?.find(p => p.id === projectId);
        if (project) {
    ...
    const submission = new ProjectSubmission({
      ...
      submissionUrl,
      fileUrl,
      attachments: skills // Store skills as attachments for now
    });
    ...
    console.log('Total projects for user:', totalProjects);
