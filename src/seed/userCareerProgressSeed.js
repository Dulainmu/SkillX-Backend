...
const seedUserCareerProgress = async () => {
  try {
    console.log('🌱 Seeding user career progress...');

    // Clear existing progress
    await UserCareerProgress.deleteMany({});
    console.log('✅ Cleared existing user career progress');

    // Get users and career roles
    const students = await User.find({ role: 'student' }).limit(15);
    ...
    console.log(`✅ Created ${savedProgress.length} user career progress records`);
    ...
    for (const achievement of achievements) {
      ...
      achievement.unlockRate = (usersWithAchievement.length / students.length) * 100;
      await achievement.save();
    }

    console.log('🎉 User career progress seeding completed!');
    return savedProgress;
  } catch (error) {
    console.error('❌ Error seeding user career progress:', error);
    throw error;
  }
};

const clearUserCareerProgress = async () => {
  try {
    await UserCareerProgress.deleteMany({});
    console.log('✅ Cleared all user career progress');
  } catch (error) {
    console.error('❌ Error clearing user career progress:', error);
    throw error;
  }
};

module.exports = {
  seedUserCareerProgress,
  clearUserCareerProgress
};
...
if (require.main === module) {
  ...
  mongoose.connect(process.env.MONGODB_URI)
    .then(() => seedUserCareerProgress())
    .then(() => {
      console.log('✅ User career progress seeding completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ User career progress seeding failed:', error);
      process.exit(1);
    });
}
