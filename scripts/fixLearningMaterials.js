...
async function fixLearningMaterials() {
  try {
    ...
    for (const role of careerRoles) {
      ...
      const roleMaterials = learningMaterials[role.name];
      const roleProjects = projectData[role.name];
      
      if (!roleMaterials || !roleProjects) {
      ...
      const updatedRoadmap = role.detailedRoadmap.map((step, index) => {
        ...
        const stepProjects = roleProjects[`step${stepNumber}`] || [];
        ...
        const updatedProjects = stepProjects.map(project => ({
          ...project,
      ...
      if (result) {
        ...
        // Verify the update
        const totalMaterials = result.detailedRoadmap.reduce((sum, step) => sum + (step.resources?.length || 0), 0);
        const totalProjects = result.detailedRoadmap.reduce((sum, step) => sum + (step.projects?.length || 0), 0);
        console.log(`  📊 Verification: ${totalMaterials} materials, ${totalProjects} projects added`);
      } else {
