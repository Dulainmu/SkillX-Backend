...
async function addLearningMaterials() {
  try {
    ...
    if (careerRoles.length === 0) {
      ...
      // Try to find any documents
      const allDocs = await mongoose.connection.db.collection('careerroles').find({}).toArray();
      console.log(`Found ${allDocs.length} documents in careerroles collection`);
    ...
    for (const role of careerRoles) {
      ...
      if (materials && projects) {
        ...
        console.log(`✅ Updated ${role.name} with ${stepMaterials.length} learning materials and ${stepProjects.length} projects`);
      } else {
    ...
    console.error('❌ Error adding learning materials:', error);
  } finally {
